...
if (isset($_POST["verify"])) {
    $email = $_SESSION['mail'];
    $result = mysqli_query($con, "SELECT OTP FROM staff WHERE Email='$email'");
...
?>
    ...
    <?php
        ...
        mysqli_query($con, "UPDATE staff SET status = 1 WHERE Email = '$email'");
    ?>
        <script>
            alert("Verify account done, you may sign in now");
            window.location.replace("staffregistration.php");
        </script>
<?php
    }
}
