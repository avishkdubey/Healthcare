...
<head>
  ...
  if (isset($_SESSION['Doctor_Id'])) { // if session already exists, redirect to doctor dashboard
  ...
  if (isset($_POST['login'])) {
    ...
    // query to retrieve doctor data with provided email and password
    $query = "SELECT * FROM doctor WHERE Email='$email' AND Password='$password' LIMIT 1";
    $result = mysqli_query($con, $query);

    if (mysqli_num_rows($result) == 1) { // if doctor exists with provided credentials
      $row = mysqli_fetch_assoc($result);

      if ($row['Status'] == 'deactive') { // check if the doctor's status is "deactive"
        ?><script>alert('You are deactive from admin. Contact admin for more info.');</script><?php
      } else {
        $_SESSION['Doctor_Id'] = $row['Doctor_Id']; // create session for doctor
        $_SESSION['Medical_Specialization'] = $row['Medical_Specialization'];

        if ($row['Status'] == '0') {
          session_destroy(); // destroy the session if the doctor's status is "deactive"
          ?><script>alert('You are deactive from admin. Contact admin for more info.');</script><?php
        } else {
          header("location: doctordash.php?doctorid=" . $_SESSION['Doctor_Id']);
          exit();
        }
      }
    } else { // if doctor doesn't exist with provided credentials
      ?><script>
            ...
            <div class="card" style="border-radius: 1rem;">
              <div class="row g-0">
                <div class="col-md-6 col-lg-5 d-none d-md-block">
                  <img src="Images/injection-1780696.jpg" alt="login form" class="img-fluid" style="border-radius: 1rem 0 0 1rem; height:550px;" />
                </div>
                  ...
                  <div class="card-body p-4 p-lg-5 text-black">
                    ...
                    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
                      ...
                      <div class="d-flex align-items-center p-0 mt-0 mb-3 pb-1">
                        ...
                        <span class="h1 fw-bold mb-0">Doctor's Login</span>
                      </div>
                      ...
                      <a class="small text-active" id="forgot" href="docform.php">Register As a Doctor </a><br>
                      <a class="small text-muted" id="forgot" href="recover-doctor-pass.php">Forgot password?</a>&nbsp;&nbsp;
